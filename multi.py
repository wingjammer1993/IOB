...
def give_training_data(filename, separator):
    ...
    with open(filename, 'r') as file_obj:
        ...
        context_tr = list(features_train)
        context_tr.insert(0, features_train[0])
        context_tr.remove(context_tr[-1])
        back_tr = list(features_train)
        back_tr.insert(0, features_train[0])
        back_tr.insert(0, features_train[0])
        back_tr.remove(context_tr[-1])
        back_tr.remove(context_tr[-1])
        front_tr = list(features_train)
        front_tr.remove(front_tr[0])
        front_tr.append(features_train[-1])
        post.append(nltk.pos_tag(features_train))
        for elem in post:
            for i in elem:
                post_train.append(i[-1])
    return features_train, labels_train, post_train, cap_tr, title_tr, lo_tr, dig_tr,  context_tr, back_tr, front_tr
...
def give_test_data(filename, separator):
    ...
    with open(filename, 'r') as file_obj:
        ...
        context_test = list(input_text)
        context_test.insert(0, input_text[0])
        context_test.remove(context_test[-1])
        back_test = list(input_text)
        back_test.insert(0, input_text[0])
        back_test.insert(0, input_text[0])
        back_test.remove(back_test[-1])
        back_test.remove(back_test[-1])
        front_test = list(input_text)
        front_test.remove(front_test[0])
        front_test.append(input_text[-1])
        post.append(nltk.pos_tag(input_text))
        for elem in post:
            for i in elem:
                post_test.append(i[-1])
    return input_text, post_test, cap_test, title_test, lo_test, dig_test, context_test, back_test, front_test
...
def extract_features(features_train, features_test, post_train, post_test, cap_train, cap_test,
                     title_train, title_test, low_train, low_test, digit_tr, digit_test, con_train, con_test,
    ...
    training_vec = sp.sparse.hstack((training_vector, context_train, bck_train, frnt_train, ptraining_vector, csr_matrix(cap_train).T,
                                     csr_matrix(title_train).T, csr_matrix(low_train).T, csr_matrix(digit_tr).T))
    test_vect = sp.sparse.hstack((test_vector, context_test, bck_test, frnt_test,  ptest_vector, csr_matrix(cap_test).T,
...
if __name__ == "__main__":
    ...
    f_t, l_t, p_t, c_t, t_t, lo_t, d_tr, s_t, b_t, fr_t = give_training_data(training, delim)
    f_test, p_test, c_test, t_Test, l_test, d_test, s_test, b_test, fr_test = give_test_data(devset, delim)
    tr_vec, test_vec = extract_features(f_t, f_test, p_t, p_test, c_t, c_test, t_t, t_Test, lo_t,
                                        l_test, d_tr, d_test, s_t, s_test, b_t, b_test, fr_t, fr_test )
